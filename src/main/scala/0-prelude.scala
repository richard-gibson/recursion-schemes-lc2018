...
object PeanoNumbers {
  ...
  val three: PeanoNumber =
    Fix(Option(Fix(Option(Fix(Option(Fix(Option.empty[PeanoNumber])))))))
...
object BinaryTrees {
  ...
  implicit val treeFFunctor: Functor[TreeF] = new Functor[TreeF] {
    override def map[A, B](fa: TreeF[A])(f: A => B): TreeF[B] =
      fa match {
        case BranchF(lbl, lf, rt) => BranchF(lbl, f(lf), f(rt))
        case LeafF(lbl)           => LeafF(lbl)
        case EmptyF()             => EmptyF()
      }
  ...
  def treeAlg: Algebra[TreeF, Tree] = {
    case BranchF(lbl, lf, rt) => Branch(lbl, lf, rt)
    case LeafF(lbl)           => Leaf(lbl)
    case EmptyF()             => Empty()
  }
  def treeCoalg: Coalgebra[TreeF, Tree] = {
    case Branch(lbl, lf, rt) => BranchF(lbl, lf, rt)
    case Leaf(lbl)           => LeafF(lbl)
    case Empty()             => EmptyF()
  }
  ...
  def toList: Algebra[TreeF, List[Int]] = {
    case BranchF(lbl, lf, rt) => lf ++ (lbl :: rt)
    case LeafF(lbl)           => List(lbl)
    case EmptyF()             => Nil
  }
  ...
  def fromList: Coalgebra[TreeF, List[Int]] = {
    ...
    case l :: Nil => LeafF(l)
    case l :: ls => {
      val (less, more) = ls partition (_ < l)
      BranchF(l, less, more)
    }
  }
